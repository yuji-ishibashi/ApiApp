...
class ApiAdapter(private val context: Context): RecyclerView.Adapter<RecyclerView.ViewHolder>(){
    ...
    private fun updateApiItemViewHolder(holder: ApiItemViewHolder, position: Int) {
        ...
        val fShop = FavoriteShop.findBy(data.id)
        isFavorite = if(fShop != null) {
            //データがある場合は削除フラグで判別（フラグが立っていたらお気に入り”ではない”）
            !(fShop.isDelete)
        } else {
            //データがない場合はお気に入りではない
            false
        ...
        holder.apply {
            rootView.apply {
                ...
                setOnClickListener {
                    onClickItem?.invoke(data.id, data.name, data.address, data.logoImage, if (data.couponUrls.sp.isNotEmpty()) data.couponUrls.sp else data.couponUrls.pc)
                }
            }
